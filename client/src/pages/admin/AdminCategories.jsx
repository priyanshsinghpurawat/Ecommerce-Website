        ...
        <div className="overflow-hidden rounded-2xl border border-white/60 bg-lux-50/40 shadow-soft backdrop-blur-md">
          <table className="w-full border-collapse text-left">
            ...
            <tbody className="divide-y divide-lux-100/40 text-xs font-semibold text-lux-dark">
              {categories.map((cat) => (
                <tr key={cat._id} className="hover:bg-lux-50/20 transition-colors">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={actionLoading}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-lux-dark py-3 font-sans text-xs font-bold uppercase tracking-wider text-black hover:bg-lux-dark-hover transition-colors disabled:opacity-50"
          >
            {actionLoading ? (
