...
import { getSellers, toggleSellerStatus } from '../../services/user.service.js';
import { Loader2, Store, User, Mail, ShieldAlert, ExternalLink, Hash } from 'lucide-react';
...
export const AdminSellers = () => {
  ...
  const handleToggleStatus = async (id) => {
    setTogglingId(id);
    try {
      const res = await toggleSellerStatus(id);
      if (res.success) {
        toast.success(res.message);
        setSellers(prev => prev.map(s => s._id === id ? { ...s, isActive: !s.isActive } : s));
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-8 pb-10">
      <div>
        <h2 className="text-2xl font-black uppercase tracking-tighter text-app-text italic">Seller Ecosystem</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {sellers.map((s) => (
                <tr key={s._id} className="hover:bg-brand-primary/[0.02] transition-colors group">
                  <td className="px-6 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="font-black text-app-text uppercase tracking-tight text-sm">{s.brandName || s.name}</span>
                        <span className="text-[9px] font-mono text-app-text/30 uppercase tracking-tighter flex items-center gap-1 mt-0.5">
                          <Hash className="h-2.5 w-2.5" /> {s._id.toString().slice(-8)}
                        </span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-6">
                    <div className="flex flex-wrap gap-1.5 max-w-[220px]">
                      {s.categories?.length > 0 ? s.categories.map((cat, idx) => (
