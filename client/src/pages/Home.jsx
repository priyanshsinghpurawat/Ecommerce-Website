...
export const Home = () => {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-6">
      <SEO
        title="Home"
        description="Premium streetwear and drip clothing for men. Shop the latest collections at MensVibe."
      ...
      {linen.length > 0 && (
        ...
        >
          <ProductShowcase
            ...
            viewAllLink={
              subBySlug.linen?._id ? `/shop?subcategory=${subBySlug.linen._id}` : '/shop'
            }
          />
        </motion.div>
      )}
      ...
      {pants.length > 0 && (
        ...
        >
          <ProductShowcase
            ...
            loading={loadingSections}
            viewAllLink={
              subBySlug.pants?._id ? `/shop?subcategory=${subBySlug.pants._id}` : '/shop'
            }
          />
        </motion.div>
      )}
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <h2 className="mt-3 text-3xl md:text-4xl font-black text-app-text tracking-tighter uppercase leading-tight">
            Redefining the <br />
            <span className="text-brand-primary">modern silhouette</span>
          </h2>
          <p className="mt-4 text-sm text-app-text/70 max-w-sm font-medium uppercase tracking-tight">
            From experimental streetwear to artisanal footwear. Crafted for those who move
            differently.
          </p>
        </div>
        <Link
        ...
        {[
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-muted font-bold uppercase tracking-tight mt-0.5">
                {text}
              </p>
            </div>
          </div>
      ...
      <div className="w-full overflow-hidden bg-white/[0.02] py-8 border-y border-white/5 select-none -mx-4 md:mx-0">
        <div className="animate-marquee whitespace-nowrap text-2xl md:text-4xl font-black uppercase tracking-[0.3em] italic flex gap-4">
          <span className="text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] animate-pulse">
            MENSVIBE STUDIO ·{' '}
            <span className="text-brand-primary drop-shadow-none">DRIP OR DROWN</span> · JAIPUR
            CREATIVE LAB ·{' '}
            <span className="text-brand-primary drop-shadow-none">LIMITED RELEASES</span> ·
