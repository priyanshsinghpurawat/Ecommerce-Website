...
import { ArrowLeft, ShoppingBag, CreditCard, Loader2, Lock, Tag, Ticket, X, Minus, Plus } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { 
  createOrderWithTax
} from '../services/order.service.js';
import { getPaymentConfig, verifyPayment } from '../services/payment.service.js';
...
export const Checkout = () => {
  const { cart, cartTotal, cartItemsCount, loading: cartLoading, updateQuantity, removeFromCart, clearCart } = useCart();
  ...
  useEffect(() => {
    if (appliedCoupon && cartItemsCount === 0) {
  ...
  const handlePlaceCodOrder = async () => {
    ...
    try {
      const res = await createOrderWithTax({
  ...
  const handleRazorpayCheckout = async () => {
    ...
    try {
      ...
      const options = {
        ...
        image: 'https://mensvibe.in/logo.png',
  ...
  useEffect(() => {
    if (appliedCoupon) {
      ...
      couponDebounceRef.current = setTimeout(async () => {
        try {
          const response = await applyCoupon(appliedCoupon.code, cartTotal * 0.18, cart.items);
  ...
  const billData = {
    baseAmount: appliedCoupon ? appliedCoupon.finalTotal : cartTotal,
    gstAmount: (appliedCoupon ? appliedCoupon.finalTotal : cartTotal) * 0.18,
    grandTotal: appliedCoupon ? appliedCoupon.finalTotal + (appliedCoupon.finalTotal * 0.18) : cartTotal + (cartTotal * 0.18)
  };
  ...
  if (cartItemsCount === 0) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center p-6">
        ...
        <h2 className="text-xl font-bold text-app-text mb-2">Your cart is empty</h2>
  ...
  return (
    <div className="space-y-8 pb-16">
      <SEO title="Checkout" description="Complete your purchase with secure checkout." noindex />
      
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black uppercase tracking-tight text-app-text">Checkout</h1>
          <p className="text-xs font-bold text-app-text/40 uppercase tracking-widest mt-1">Complete your purchase securely</p>
        </div>
        <Link
          to="/cart"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-surface-50/40 shadow-soft backdrop-blur-md">
            <div className="p-6 border-b border-border-base bg-surface-50/30">
              <h3 className="text-xs font-black uppercase tracking-wider text-app-text">Order Items ({cartItemsCount})</h3>
            </div>
              ...
              <table className="w-full border-collapse text-left">
                ...
                <tbody className="divide-y divide-surface-100/40 text-xs font-bold text-app-text">
                  {cart.items.map((item) => {
                    ...
                    return (
                      <tr key={item._id} className="hover:bg-surface-50/20 transition-all">
        ...
        <div className="rounded-3xl border border-white/10 bg-surface-50/40 p-6 shadow-soft backdrop-blur-md space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {shippingFields.map(({ key, label, type, maxLength }) => (
              <div key={key} className={key === 'street' ? 'sm:col-span-2' : ''}>n                <label className="text-[10px] font-black uppercase tracking-widest text-app-text/50 block mb-1">{label}</label>
                <input
                  type={type}
