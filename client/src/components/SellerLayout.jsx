...
export const SellerLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-lux-bg text-lux-dark font-sans overflow-hidden">
        ...
        <div className="h-[72px] flex items-center px-6 border-b border-border-base">
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-sm font-black tracking-tight text-lux-dark uppercase">
              Mens<span className="italic text-lux-primary">Vibe</span> <span className="text-[9px] bg-lux-primary text-black px-1.5 py-0.5 rounded-full ml-1 font-black">SELLER</span>
            </span>
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-border-base bg-lux-dark/5">
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <div className="truncate flex-1">
              <p className="text-sm font-black italic text-lux-dark truncate uppercase tracking-tighter">{user?.name || 'Seller User'}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-lux-bg">
        <header className="h-[72px] flex items-center justify-between px-8 bg-lux-bg border-b border-border-base z-10">
          <div className="flex items-center gap-4">
            <button onClick={() => setMobileMenuOpen(true)} className="md:hidden p-2 rounded-xl text-muted hover:bg-lux-100">
              <Menu className="h-5 w-5" />
            </button>
            <h1 className="text-xl font-black uppercase tracking-widest text-lux-dark italic hidden md:block">
              Seller <span className="text-lux-primary">Control</span>
            </h1>
          </div>

          <div className="flex items-center gap-4">
             <Link to="/seller/products" className="flex items-center gap-2 px-4 py-2 bg-lux-dark text-lux-bg rounded-xl font-black text-[10px] uppercase tracking-wider hover:opacity-90 transition-all shadow-lg">
      ...
      {mobileMenuOpen && (
         <div className="fixed inset-0 z-50 md:hidden">
            <div className="fixed inset-0 bg-lux-bg/40 backdrop-blur-md" onClick={() => setMobileMenuOpen(false)} />
            <aside className="relative flex flex-col w-72 max-w-xs h-full bg-lux-100 shadow-2xl transition-all duration-300 animate-in slide-in-from-left">
               <div className="h-[72px] flex items-center justify-between px-6 border-b border-border-base">
                  <span className="text-sm font-black uppercase tracking-widest text-lux-dark">
                    Mens<span className="italic text-lux-primary">Vibe</span> Seller
                  </span>
                  <button onClick={() => setMobileMenuOpen(false)}><X className="h-5 w-5 text-muted hover:text-lux-dark" /></button>
               </div>
               <nav className="flex-1 px-4 py-6 space-y-1.5">
                  {navLinks.map((link) => (
                    <NavLink key={link.to} to={link.to} onClick={() => setMobileMenuOpen(false)} className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${isActive ? 'bg-lux-primary text-black shadow-md' : 'text-muted hover:bg-lux-200 hover:text-lux-dark'}`}>
